...
public class K_ReturnToHome_OVR : MonoBehaviourPun
{
    [Header("Home(원위치)")]
    ...
    [Header("Held 중 손과 충돌 무시")]
    public bool ignoreHandCollisionsWhileHeld = true;

    [Header("Held 중 플레이어 캡슐 충돌 무시")]
    ...
    {
        // [수정된 부분]
        // 포톤 뷰가 있고, 내가 주인도 아니고(AND), 내가 잡고 있는 상태도 아니라면(AND) -> 리턴
        // 즉, "내가 잡고 있다면" 주인이 아직 아니어도 아래 로직(충돌 무시 등)을 실행함.
        if (photonView != null && !photonView.IsMine && !grabbable.isGrabbed) return;
    ...
    {
        // [수정된 부분] 충돌 감지 역시 내가 잡고 있거나 내꺼일 때만 처리
        if (photonView != null && !photonView.IsMine && !grabbable.isGrabbed) return;
